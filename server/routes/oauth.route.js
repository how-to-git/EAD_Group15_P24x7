...
router.get(
  "/github/auth/connect",
  passport.authenticate("connectGitHub", { scope: ["user:email"] })
);
...
router.get("/github", async (req, res, next) => {
  passport.authenticate("github", (err, user, info) => {
    if (err)
    ...
    else if (info && (info.status ? (info.status >= 400 ? true : false) : true))
      return res
      ...
      return res
        ...
        .send(
          JSON.stringify(
            { status: info.status, type: info.type, url: info.url },
            null,
            2
          )
        );
    ...
    if (err)
      ...
      passport.authenticate("connectGitHub", (err, github, info) => {
        if (err)
          ...
          info &&
          (info.status ? (info.status >= 400 ? true : false) : true)
        )
          return res
            .status(info.status ? info.status : (info.status = 400))
            .send(
...
router.get(
  "/google/auth/connect",
  passport.authenticate("connectGoogle", { scope: ["email"] })
);
...
router.post("/:provider/validate", async (req, res, next) => {
  ...
  passport.authenticate("getOAuthData", (err, user, info) => {
    if (err)
      ...
      return res.send(
        JSON.stringify(
          { status: info.status, userExists: info.message },
          null,
          2
        )
      );
