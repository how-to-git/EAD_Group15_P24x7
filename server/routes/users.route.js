...
const GenerateLimiter = new rateLimit({
  ...
  handler: (req, res) =>
    res
      .status(429)
      .send(JSON.stringify({ status: 429, message: MSG_DESC[30] }, null, 2)),
});
...
router.post("/register", async (req, res, next) => {
  passport.authenticate("register", (err, user, infos) => {
    if (err)
      return res
        .status(500)
        .send(JSON.stringify({ status: 500, message: MSG_DESC[0] }, null, 2));
    else if (
      infos &&
      (infos.status ? (infos.status >= 300 ? true : false) : true)
    )
      return res
        .status(infos.status ? infos.status : (infos.status = 400))
        .send(JSON.stringify(infos, null, 2));
    else if (
      user &&
      (req.body = { email: req.body.email, id: String(user._id) })
    ) {
        ...
        (err, user, info) => {
          if (err)
            return res
              .status(500)
              .send(
                JSON.stringify({ status: 500, message: MSG_DESC[0] }, null, 2)
              );
          else if (
            info &&
            (info.status ? (info.status >= 300 ? true : false) : true)
          )
            return res
              .status(info.status ? info.status : (info.status = 400))
              .send(JSON.stringify({ info, message: info.message }, null, 2));
          else if (user)
            return res
              .cookie(
                "jwt-token",
            ...
            return res
              .status(504)
              .send(
                JSON.stringify({ status: 504, message: MSG_DESC[34] }, null, 2)
              );
        }
      )(req, res, next);
    } else
      return res
        .status(504)
        .send(JSON.stringify({ status: 504, message: MSG_DESC[34] }, null, 2));
  })(req, res, next);
});
  ...
  passport.authenticate("login", (err, user, info) => {
    if (err)
      return res
        .status(500)
        .send(JSON.stringify({ status: 500, message: MSG_DESC[0] }, null, 2));
    else if (info && (info.status ? (info.status >= 300 ? true : false) : true))
      return res
        .status(info.status ? info.status : (info.status = 400))
        .send(
          JSON.stringify(
        ...
        if (err)
          ...
          return res
            .cookie(
              ...
              {
                path: "/",
                expires: JSON.parse(req.body.rememberMe)
...
router.get("/user", async (req, res, next) => {
  passport.authenticate("jwt", { session: false }, (err, user, info) => {
    ...
    else if (info && info.status === 302)
      return res
        .status(info.status)
        .send(
          JSON.stringify({ ...info, "XSRF-TOKEN": req.csrfToken() }, null, 2)
        );
    else if (info && (info.status ? (info.status >= 300 ? true : false) : true))
      return res
        .status(info.status ? info.status : (info.status = 400))
        .send(
          JSON.stringify(
            { message: info.message, ...info, "XSRF-TOKEN": req.csrfToken() },
            null,
            2
          )
        );
  ...
  passport.authenticate("jwt", { session: false }, (err, user, info) => {
    if (err)
      ...
      if (user.security["2FA"] && (req.body = { ...req.body, ...user._doc })) {
                ...
                (err, account, info) => {
                  if (err)
                    ...
                    return res
                      ...
                      .send(
                        JSON.stringify(
                          { status: 504, message: MSG_DESC[34] },
                          null,
                          2
                        )
                      );
        ...
        user &&
        (req.body = {
          ...(req.body && (req.body = { ...req.body, ...user._doc })),
        })
      ) {
        passport.authenticate(
          "changePassword",
      ...
      passport.authenticate(
        ...
        (err, token, info) => {
          if (err)
            ...
            return res.status(info.status).send(JSON.stringify(info, null, 2));
          else
            return res
              .status(504)
              .send(
