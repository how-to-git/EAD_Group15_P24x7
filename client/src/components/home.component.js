...
const parseLabel = (a) => {
  ...
  if (a[1]) {
    if (a[0] + " " + a[1] === labels[3]) _labelClass = "do-later";
  } else {
...
const Home = ({ userData }) => {
  ...
  async function getTodoData() {
    await axios
      .get(`${SERVER_URL}/todo/data`, { withCredentials: true })
      .then((res) => {
  ...
  const addTodo = (e) => {
    ...
    async function submitData() {
      ...
      await axios
        .post(`${SERVER_URL}/todo/data`, data, {
    ...
    if (properties.honeypot) return;
    else if (!data.title || !data.date || !data.label) {
      setNotification(
        NOTIFICATION_TYPES.DANGER,
        "Please Make Sure to Fill Out All the Required Fields !"
      );
      document
        .getElementById(!data.title ? "title" : !data.date ? "date" : "label")
        .focus();
    } else if (data.title.length > 60) {
      setNotification(
        NOTIFICATION_TYPES.DANGER,
        "Please Provide a Title less than 60 characters !"
      );
      document.getElementById("title").focus();
    } else if (validateLabel(data.label)) {
      setNotification(
        NOTIFICATION_TYPES.DANGER,
      ...
      setNotification(
        NOTIFICATION_TYPES.DANGER,
        "Please Provide a Description Less than 200 characters !"
      );
    ...
    <div className="main__projects">
      ...
      <Tooltip title="Add Task" placement="top">
        <button
          ...
          aria-label="Add Todo"
          onClick={() =>
            authenticated ? openModal("background", "modal", "title") : null
          }
          id="addTodo"
          style={{ bottom: "17vh" }}
        >
        ...
        <div id="modal" className="modal__container hiddenModal">
          <div className="modal__title">
            <span
              ...
              onClick={() => closeModal("background", "modal")}
            >
              &times;
            </span>
            <h2>Add Todo</h2>
              ...
              <div className="m-10">
                <div className="contact__infoField">
                  ...
                  <Select
                    ...
                    className="mt-10 mb-10 full-width"
                  >
                    {labels.map((c) => {
                      return <MenuItem value={c.toLowerCase()}>{c}</MenuItem>;
                    })}
                  </Select>
                </div>
              </div>
